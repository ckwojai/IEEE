...
void setup() {
  ...
  setData(&qdata, 0, 0, 75*75, 75*75, 75*75, 75*75);
...
void controlSpeed(struct apdata* sp) {
  analogWrite(m1pin, sqrt(sp->m1sp));
  analogWrite(m2pin, sqrt(sp->m2sp));
  analogWrite(m3pin, sqrt(sp->m3sp));
  analogWrite(m4pin, sqrt(sp->m4sp));
}
